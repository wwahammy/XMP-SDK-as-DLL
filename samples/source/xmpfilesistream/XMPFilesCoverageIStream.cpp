...
static void OpenTestFile ( const char * fileName, XMP_OptionBits rwMode, SXMPMeta* xmpMeta, SXMPFiles* xmpFile, XMP_FileFormat format )
{
	bool ok;
	...
	HRESULT hr = SHCreateStreamOnFileEx(wcstring, STGM_READWRITE|STGM_TRANSACTED, NULL, FALSE, NULL, &stream);

	
	


	 ok = xmpFile->OpenFile ( stream, format, smartFlags );
	if ( ! ok ) {
...
static void TestOneFile ( const char * fileName, XMP_FileFormat format )
{
	char buffer [1000];
	SXMPMeta  xmpMeta;	
	SXMPFiles xmpFile;
	...
	OpenTestFile ( fileName, kXMPFiles_OpenForRead, &xmpMeta, &xmpFile, format );
	...
	OpenTestFile ( fileName, kXMPFiles_OpenForUpdate, &xmpMeta, &xmpFile, format );
...
extern "C" int main ( int argc, const char * argv[] )
{
	...
	time_t now = time(0);
	fprintf ( sLogFile, "XMPFilesCoverageIStream starting %s", ctime(&now) );
	...
	try {
		...
		TestOneFile ( "../../../testfiles/BlueSquare.avi", kXMP_AVIFile );
		/*
